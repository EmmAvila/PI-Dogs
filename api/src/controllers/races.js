...
let getRaces = (req, res, next) => {
    ...
    Promise.all([datApi, datDb]).then(resp => {
        ...
        // racesApi.forEach((e,i)=> {
          // console.log(raceDb[0].dataValues.Tempers[0].dataValues.name)
          // console.log('hola', raceDb[0].Tempers[0].name)
        // })
        // let raceDb = raceDataBase.map(r=>)
